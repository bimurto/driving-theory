...
export default async function ChapterPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const adjacent = getAdjacentChapters(catalog.chapters, chapter.slug);
  return <section className="content page-content guide"><Link className="button secondary topics-link" href="/topics">← Back to topics</Link><p className="eyebrow">{chapter.themeName}</p><h1>{chapter.chapterNumber} — {chapter.chapterName}</h1><p className="lede">{chapter.questions.length} Class B questions in this chapter.</p><Link className="button" href={`/practice?chapter=${chapter.slug}`}>Practise this chapter</Link>{chapter.summary ? <div className="guide-body"><aside className="guide-toc"><strong>In this guide</strong><nav aria-label="Guide sections"><ul>{toc.map((heading) => <li key={heading}><a href={`#${slugify(heading)}`}>{heading}</a></li>)}</ul></nav></aside><article className="summary"><ReactMarkdown remarkPlugins={[remarkGfm]} components={{ h2: ({ children }) => <h2 id={slugify(String(children))}>{children}</h2>, h3: ({ children }) => <h3 id={slugify(String(children))}>{children}</h3>, img: ({ src, alt }) => <img src={typeof src === "string" ? resolveMarkdownImageUrl(src) : undefined} alt={alt ?? ""} /> }}>{chapter.summary}</ReactMarkdown></article></div> : <p className="notice">This chapter can be practised now; its study guide is being prepared.</p>}<ChapterQuestionList chapter={chapter} /><nav className="chapter-navigation" aria-label="Chapter navigation">{adjacent.previous ? <Link className="chapter-navigation-link previous" href={`/topics/${adjacent.previous.slug}`}><span>← Previous chapter</span><strong>{adjacent.previous.chapterNumber} — {adjacent.previous.chapterName}</strong></Link> : <span />}{adjacent.next ? <Link className="chapter-navigation-link next" href={`/topics/${adjacent.next.slug}`}><span>Next chapter →</span><strong>{adjacent.next.chapterNumber} — {adjacent.next.chapterName}</strong></Link> : <Link className="chapter-navigation-link next" href="/topics"><span>End of catalogue</span><strong>Back to all chapters →</strong></Link>}</nav></section>;
}
