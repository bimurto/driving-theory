...
export function ProgressPanel() {
  ...
  return <section className="content page-content"><p className="eyebrow">Your learning</p><h1>Progress</h1><div className="progress-tabs">{(["overview", "failed", "starred", "notes"] as const).map((item) => <button key={item} className={tab === item ? "active" : ""} onClick={() => setTab(item)}>{item === "failed" ? "Failed answers" : item[0].toUpperCase() + item.slice(1)}</button>)}</div>{tab === "overview" && <><div className="stat-grid"><div><strong>{values.length}</strong><span>of {allQuestions.length} studied</span></div><div><strong>{attempts ? `${Math.round(correct / attempts * 100)}%` : "—"}</strong><span>accuracy</span></div><div><strong>{failed}</strong><span>failed answers</span></div></div><section className="progress-breakdown"><h2>By topic</h2><div className="topic-progress-list">{Object.entries(themes).map(([theme, questions]) => {
    ...
    return <div className="topic-progress" key={theme}><strong>{theme}</strong><span>{records.length}/{questions.length} studied</span><span>{themeAttempts ? `${Math.round(themeCorrect / themeAttempts * 100)}% correct` : "Not attempted"}</span></div>;
